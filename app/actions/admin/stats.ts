...
export async function getAdminStats() {
  ...
  async function getAnalyticsData(timeRange: string) {
    try {
      ...
      return {
        analytics: { pageViews: { total: 0, change: 0, data: [] }, topPages: [], referrers: [], countries: [], events: [] },
        environment: envConfig.NODE_ENV,
        enabled: analyticsConfig.enabled,
      }
    } catch {
  ...
  if (!analyticsData) {
    ...
    const [last7Count, prev7Count, last7Views, topPagesRaw, referrersRaw] = await Promise.all([
      ...
      prisma.pageView.findMany({ where: { timestamp: { gte: oneWeekAgo } }, select: { timestamp: true } }),
      prisma.pageView.groupBy({ by: ["page"], _count: { page: true }, orderBy: { _count: { page: "desc" } }, take: 5 }),
      prisma.pageView.groupBy({ by: ["referrer"], where: { referrer: { not: null } }, _count: { referrer: true }, orderBy: { _count: { referrer: "desc" } }, take: 5 }),
    ])
  ...
  return {
    ...
    blog: { published: publishedPosts, drafts: draftPosts, totalViews: totalBlogViews._sum.views || 0 },
    analytics: (analyticsData
      ? { ...analyticsData.analytics, environment: envConfig.NODE_ENV, enabled: analyticsData.enabled }
      : analyticsFromDb
      ? { ...analyticsFromDb, environment: envConfig.NODE_ENV, enabled: true }
      : { pageViews: { total: pageViews, change: 0, data: [] }, topPages: [], referrers: [], countries: [], events: [], environment: envConfig.NODE_ENV, enabled: false }),
    database,
  }
}
